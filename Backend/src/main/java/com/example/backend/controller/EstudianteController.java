...
public class EstudianteController {
  ...
  public ResponseEntity<Object> login(@RequestBody HashMap<String, String> request) {
    ...
    return estudianteList.stream().anyMatch(estudiante -> estudiante.getEmail().equals(email))
        ? estudianteService.login(email)
        : ResponseEntity.status(HttpStatus.NOT_FOUND).body(EMAIL_NOT_REGISTERED);
  }
}
